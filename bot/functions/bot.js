// bot/functions/bot.js (Обновлено: условный /start, удаление сообщений)
...
// --- ИЗМЕНЕНО: getOrCreateUser возвращает { id, claimed, lastMessageId } ---
async function getOrCreateUser(supabase, userId) {
    ...
    try {
        let { data: existingUser, error: selectError } = await supabase
            .from('users')
            .select('id, channel_reward_claimed, last_start_message_id') // <<<--- Добавили поля
            .eq('tg_id', userId)
            .single();
        ...
        if (existingUser) {
            ...
            const { data: newUser, error: insertError } = await supabase
                .from('users')
                .insert({ tg_id: userId }) // channel_reward_claimed = false (DEFAULT), last_start_message_id = NULL (DEFAULT)
                .select('id')
                .single();
            ...
            return { // <<<--- Возвращаем объект для нового пользователя
                id: newUser.id,
                claimed: false, // Новый пользователь не получал награду
                lastMessageId: null // У нового пользователя нет прошлого сообщения
            };
        }
    } catch (error) {
...
// ... (Функции getGeminiAnalysis, analyzeDream, logReplyError остаются БЕЗ ИЗМЕНЕНИЙ) ...
...
// --- ИЗМЕНЕНО: Логика bot.command("start") ---
bot.command("start", async (ctx) => {
    ...
    let userData;
    try {
        // 1. Получаем данные пользователя (включая last_start_message_id)
        ...
        // 2. Пытаемся удалить предыдущее сообщение /start, если оно было
        if (userData.lastMessageId) {
            ...
            try {
                ...
                if (deleteError instanceof GrammyError && (deleteError.error_code === 400 || deleteError.description.includes("message to delete not found"))) {
                    ...
                    // Логируем другие ошибки удаления, но не прерываем выполнение
                    console.error(`[Bot Handler /start] Failed to delete message ${userData.lastMessageId}:`, deleteError);
                }
            }
        }

        // 3. Определяем сообщение и URL кнопки в зависимости от статуса награды
        let messageText;
        let buttonText;
        let buttonUrl;
        ...
        if (userData.claimed) {
            ...
            buttonUrl = TMA_URL; // Обычный URL без ?action=claim_reward
        } else {
            // Награда еще не получена
            ...
            buttonUrl = `${TMA_URL}?action=claim_reward`; // URL с параметром
        }
        ...
        // 5. Сохраняем ID нового сообщения в базу данных
        const { error: updateError } = await supabaseAdmin
            .from('users')
            .update({ last_start_message_id: sentMessage.message_id })
            .eq('id', userData.id); // Обновляем по внутреннему ID

        if (updateError) {
            // Логируем ошибку, но не считаем это критичным для пользователя
        ...
        try {
            // Пытаемся уведомить пользователя об ошибке, но без сохранения ID этого сообщения
